...
const Paid = ({ search, data }) => {
    ...
    const { loading, error, setData } = useFetch('https://wayback.up.railway.app/paids')
                ...
                <div className="row pb-4">
...

                    {
                        data.filter((props) => {
                            if (search === '') {
                                return props
                            } else if (props.longname.toLowerCase().includes(search.toLowerCase())) {
                                return props
                            }
                        }).map((props) => (
                                ...
                                <Link to={`/detail/${props.id}`} style={{ textDecoration: 'none' }} >
                                    <div>
                                        <div className='imageHolder55'>
                                            <img src={`https://wayback.up.railway.app${props.sight.url}` || a} alt="" className='mama' />
                                        </div>
